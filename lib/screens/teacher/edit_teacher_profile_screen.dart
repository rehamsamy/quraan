...
class EditTeacherProfile extends StatefulWidget {
  ...
  EditTeacherProfile({Key? key, required this.studentID}) : super(key: key);
...
class _EditTeacherProfileState extends State<EditTeacherProfile> {
  ...
  getUserDataFromUrl(studentID) async {
    ...
    try {
      var response =
          await Dio().get("${AppConstance.api_url}/teacher/${studentID}",
              options: Options(headers: {
                'Authorization':
                    'Bearer ${json.decode(prefs.getString("tokenAccess")!)}',
              }));
